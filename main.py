...
# --- 1. การตั้งค่าหน้าตาแอป (Classic White & Big Button) ---
st.set_page_config(page_title="Rin Secretary v35.0", layout="centered")
...
show_rin()
st.markdown("<h3 style='text-align:center;'>Rin Secretary v35.0 (Vision)</h3>", unsafe_allow_html=True)
...
if audio:
    # ใช้ Gemini แปลงเสียง (Whisper เดิม)
    import groq # ใช้ Groq แปลงเสียงเพราะไวกว่าค่ะ
...
if final_input:
    ...
    with st.chat_message("assistant", avatar="👓"):
        if any(w in final_input for w in ["จด", "บันทึก", "จำ"]):
            ...
            else: answer = "รินจดไม่ได้ค่ะ เช็คสิทธิ์ Sheets หน่อยนะ คะ"
        else:
            with st.spinner("รินกำลังใช้ดวงตาและสมองประมวลผล..."):
                ...
                # เรียกใช้สมอง Gemini 1.5 Flash (Vision Support)
