...
st.set_page_config(page_title="Rin v34.8 Partner - ML Upgrade", layout="centered")
...
async def make_voice(text):
    # ปรับจังหวะให้ดูนิ่งและเป็นทางการแบบเดอาน่ามากขึ้น
...
with st.sidebar:
    ...
    st.divider()
    search_mode = st.toggle("🔍 โหมดหาข้อมูล (Tavily)", value=False)
...
# --- 4. Perception Layer (รับรูปภาพและเสียง) ---
with st.container():
    uploaded_file = st.file_uploader("ส่งรูปภาพให้รินวิเคราะห์ (บิล, กราฟ, หรือโฆษณา)", type=["jpg", "jpeg", "png"])
    col_mic, col_input = st.columns([1, 6])
    with col_mic: audio = audio_recorder(text="", icon_size="2x", neutral_color="#444444")
    prompt = st.chat_input("คุยกับริน...")
...
if final_input or uploaded_file:
    ...
    if uploaded_file:
        ...
        st.image(uploaded_file, caption="รูปที่ส่งให้ริน", width=300)

    st.session_state.messages.append({"role": "user", "content": final_input if final_input else "วิเคราะห์รูปนี้ให้หน่อยค่ะ"})
    with st.chat_message("user"): st.markdown(final_input if final_input else "*(ส่งรูปภาพ)*")

    with st.chat_message("assistant", avatar="👓"):
        with st.spinner("รินกำลังใช้ Machine Learning ประมวลผล..."):
            search_context = ""
            if search_mode:
                try:
                    tavily = TavilyClient(api_key=st.secrets["TAVILY_API_KEY"])
                    res = tavily.search(query=final_input if final_input else "วิเคราะห์รูปภาพ", max_results=3)
                    search_context = "\nข้อมูลเสริมจากเน็ต: " + " ".join([r['content'] for r in res['results']])
                except: pass

            try:
                client = Groq(api_key=st.secrets["GROQ_API_KEY"])
                
                # เลือกรุ่น ML ตามประเภทข้อมูล (Upgrade Point!)
                model_name = "llama-3.2-11b-vision-preview" if uploaded_file else "llama-3.3-70b-versatile"
                
                sys_msg = f"คุณคือ 'ริน' AI คู่หูอัจฉริยะ (Diana Mode) ของบอสคิริลิ ดูแลสุขภาพ บิล และการตลาด บุคลิกนิ่ง สุขุม ฉลาด ลงท้าย ค่ะ/คะ เสมอ {search_context}"
                ...
                answer = f"ระบบ ML ขัดข้องเล็กน้อยค่ะบอส: {str(e)}"
