...
st.set_page_config(page_title="Rin v34.9.2 Partner", layout="centered", initial_sidebar_state="expanded")
...
def read_last_memory(limit=5):
    try:
        ...
        return "\n".join([f"- บอสเคยสั่งว่า: {r['fields'].get('User')}" for r in records])
...
show_rin()
st.markdown("<h3 style='text-align:center;'>Rin v34.9.2 Partner</h3>", unsafe_allow_html=True)
...
if audio:
    with st.spinner("รินฟังอยู่นะคะ..."):
        ...
        except: st.error("ไมค์มีปัญหาค่ะ")
...
if final_input:
    ...
    with st.chat_message("assistant", avatar="👓"):
        ...
        client = Groq(api_key=st.secrets["GROQ_API_KEY"])
        system_prompt = f"คุณคือ 'ริน' เลขาบอสคิริลิ ความจำล่าสุด: {past_memory} ข้อมูลเน็ต: {context} ตอบบอสอย่างฉลาด หวานๆ ลงท้ายค่ะ/คะ"
