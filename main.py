...
st.set_page_config(page_title="Rin Secretary v35.3", layout="centered")
...
st.markdown("""
    ...
    * { color: #000000 !important; font-size: 21px !important; }
    ...
    /* ปุ่มเมนูวงกลมสีม่วงขนาดใหญ่ (Big Sidebar Button) */
...
# --- 2. ระบบเช็กกุญแจสมอง Gemini ---
try:
    # ใช้ชื่อกุญแจที่ตั้งไว้ใน Secrets
    genai.configure(api_key=st.secrets["GOOGLE_API_KEY"])
except Exception as e:
    st.error("👓 บอสคะ! รินหากุญแจ GOOGLE_API_KEY ไม่เจอในหน้า Secrets ค่ะ รบกวนบอสเช็กอีกทีนะ คะ")
    st.stop()
...
show_rin()
st.markdown("<h3 style='text-align:center;'>Rin Secretary v35.3</h3>", unsafe_allow_html=True)

# ส่วนอัปโหลดรูปภาพ (Vision)
...
if final_input:
    ...
    with st.chat_message("assistant", avatar="👓"):
        if any(w in final_input for w in ["จด", "บันทึก", "จำ"]):
            ...
            else: answer = "รินจดไม่ได้ค่ะ เช็กสิทธิ์ใน Google Sheets หน่อยนะ คะ"
        else:
            with st.spinner("รินกำลังใช้สมองประมวลผล..."):
                tavily = TavilyClient(api_key=st.secrets["TAVILY_API_KEY"])
                context = ""
                # ระบบค้นหาจะทำงานเมื่อเลือก Max หรือถามราคา/ข่าว
                ...
                content_parts = [f"คุณคือริน เลขาบอสคิริลิ ข้อมูลเสริม: {context} ตอบหวานๆ ลงท้ายด้วยค่ะ/คะ"]
                ...
                content_parts.append(f"User input: {final_input}")
                
                try:
                    response = model.generate_content(content_parts)
                    answer = response.text
                except Exception as e:
                    answer = f"ขอโทษค่ะบอส สมองรินมึนไปนิดนึง ลองใหม่อีกทีนะ คะ (Error: {e})"
